...
@export var resource_version: int
@export var syntax_version: int
...
func get_next_dialogue_line(title: String, extra_game_states: Array = []) -> Dictionary:
	...
	if tree:
		...
		if dialogue_manager != null:
			return await dialogue_manager.get_next_dialogue_line(title, self, extra_game_states).completed
