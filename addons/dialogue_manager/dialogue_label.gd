...
func reset_height() -> void:
	...
	size_check_label.size.x = size.x
	size_check_label.text = dialogue_line.text
	
	# Give the size check a chance to resize
	await get_tree().idle_frame
	
	# Resize our dialogue label with the new size hint
	custom_minimum_size = Vector2(size.x, size_check_label.get_content_height())
	size = Vector2.ZERO
...
func type_next(delta: float, seconds_needed: float) -> void:
	...
	if last_wait_index != index and get_pause(index) > 0:
		...
		index += 1
		seconds_needed += seconds_per_step * (1.0 / get_velocity(index))
		if seconds_needed > delta:
			waiting_seconds += seconds_needed
			if index < text.length():
				emit_signal("spoke", text[index - 1], get_velocity(index))
		else:
