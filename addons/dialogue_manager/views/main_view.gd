...
func _ready() -> void:
	...
	update_button.add_theme_color_override("font_color", get_color("success_color", "Editor"))
	...
	insert_menu.get_popup().connect("id_pressed",Callable(self,"_on_insert_menu_id_pressed"))
	...
	translations_menu.get_popup().connect("id_pressed",Callable(self,"_on_translation_menu_id_pressed"))
...
func build_open_menu() -> void:
	...
	if menu.is_connected("index_pressed",Callable(self,"_on_open_menu_index_pressed")):
		menu.disconnect("index_pressed",Callable(self,"_on_open_menu_index_pressed"))
	menu.connect("index_pressed",Callable(self,"_on_open_menu_index_pressed"))
...
func set_resource(value: DialogueResource) -> void:
	...
	if current_resource:
		...
		if cursors.has(current_resource.resource_path):
			...
			editor.set_caret_line(cursor.y, true)
			editor.set_caret_column(cursor.x, true)
		...
		while !file.eof_reached():
			...
			if line.begins_with("msgid"): # Extract msgid
				...
				if msgid in id_str:
					...
					# skip old msgstr
					while !file.eof_reached() and !line.is_empty() and (line.begins_with("msgstr") or line.begins_with("\"")):
						line = file.get_line().strip_edges()
					existing_po += line + "\n"
...
func _on_CodeEditor_active_title_changed(title):
	...
	run_node_button.tooltip_text = "Play the test scene using \"%s\"" % title
...
func _on_CodeEditor_cursor_changed():
	...
	next_resource_cursors[current_resource.resource_path] = { 
		x = editor.get_caret_column(), 
		y = editor.get_caret_line() 
	}
...
func _on_ErrorList_error_pressed(error):
	editor.set_caret_line(error.get("line"))
