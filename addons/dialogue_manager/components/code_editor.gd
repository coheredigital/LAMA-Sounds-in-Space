...
func _ready() -> void:
	# Set up popup menu
	var menu = get_menu()
	menu.connect("about_to_popup",Callable(self,"_on_menu_about_to_show"))
	menu.connect("index_pressed",Callable(self,"_on_menu_index_pressed"))
	menu.add_separator()
	...
	choose_title_dialog.connect("title_chosen",Callable(self,"_on_title_chosen"))
...
func set_colors_from_editor(editor_settings: EditorSettings) -> void:
	...
	# Errors
	add_theme_color_override("bookmark_color", editor_settings.get_setting("text_editor/highlighting/brace_mismatch_color"))
	
	# General UI
	add_theme_constant_override("line_spacing", 10)
	add_theme_color_override("symbol_color", editor_settings.get_setting("text_editor/highlighting/symbol_color"))
	add_theme_color_override("font_color", editor_settings.get_setting("text_editor/highlighting/text_color"))
	add_theme_color_override("background_color", editor_settings.get_setting("text_editor/highlighting/background_color"))
	add_theme_color_override("current_line_color", editor_settings.get_setting("text_editor/highlighting/current_line_color"))
	add_theme_font_override("font", get_font("source", "EditorFonts"))
...
func set_cursor(cursor: Vector2) -> void:
	set_caret_line(cursor.y, true)
	set_caret_column(cursor.x, true)
