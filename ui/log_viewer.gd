...
func _on_event_logger_line_added(unix_time: float,type: String,action: String,info: String):
	var time_formatted = Time.get_datetime_dict_from_unix_time(unix_time)
	var time_string = "%02d:%02d:%02d" %[time_formatted.hour,time_formatted.minute,time_formatted.second]
...
func add_line(time_string: String,type: String,action: String,info: String):
	...
	text = text + "%-12s%-24s\"%s\"" % [time_string,type_action_string,info]
